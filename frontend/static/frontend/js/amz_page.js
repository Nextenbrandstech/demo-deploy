...
// ──────────────────────────────────────────────
// FUNCTION: Fetch Insights & Update Cards
// ──────────────────────────────────────────────
function fetchInsights(startDate, endDate, seller, brand) {
  const url = `/backend/api/amz_insights/?start_date=${startDate}&end_date=${endDate}&seller=${seller}&brand=${brand}`;
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      // This is for Display Card values
  ...
  `;
    ...
    `;
        ...
        // This is for Trend-lines graphs
        ...
        // This is for Product Detail table
...
// ──────────────────────────────────────────────
// EVENT LISTENERS: Update Data on User Input
// ──────────────────────────────────────────────
...
// This function is to update the plots
...
const SellerBrandMap = {
  ...
  "TRI": [""],
  "Amour Hygiene": [""],
  "Lifeway Hygiene": ["Little Smile", "Elderly Smile"],
  "All Seller": [""]
...
const currentKeys = [
  "gross_revenue", "gross_units", "cancelled_amt", "cancelled_units", "returned_amt", "returned_units", "refund_commission_fee",
  "net_revenue_with_tax", "net_units", "tax", "net_revenue_without_tax", "cogs", "product_margin", "codb",
  "weight_handling_fee", "fixed_closing_fee", "referral_fee", "cm_1", "operations", "cm_2", "ads_spend", "cm3",
  "profit_percentage"
];

// Keys that should be collapsible under "codb"
const collapsibleKeys = [
  "weight_handling_fee", "fixed_closing_fee", "referral_fee"
];
...
function fetchPnLData() {
  ...
  const url = `/backend/api/amz_pnl_details/?pnl_start_date=${startDate}&pnl_end_date=${endDate}&time_stamp=${timeFormat}&seller=${seller}&brand=${brand}`;
  ...
  finalKeys.forEach(param => {
    ...
    dataKeys.forEach(colKey => {
      ...
      if (param === "cm3") {
        ...
        // The below implementation was by default and it takes everything as a cost,
        // cellValue = backendCM3 - totalCustomPerColumn;
